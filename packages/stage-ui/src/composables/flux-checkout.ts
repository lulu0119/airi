...
/**
 * Optional hooks for Stripe Checkout analytics (e.g. PostHog) on non-iOS builds.
 * Hooks are not used on the iOS / Apple IAP path.
 */
export interface FluxCheckoutStripeHooks {
  onStripePlanSelected?: (priceId: string) => void
  onStripeBeforeRedirect?: (priceId: string) => void
}
...
export function useFluxCheckout() {
  ...
  async function purchaseStripe(priceId: string, stripeHooks?: FluxCheckoutStripeHooks): Promise<PurchaseResult> {
    stripeHooks?.onStripePlanSelected?.(priceId)
    ...
    stripeHooks?.onStripeBeforeRedirect?.(priceId)
  ...
  async function purchase(id: string, stripeHooks?: FluxCheckoutStripeHooks): Promise<PurchaseResult> {
    if (purchasing.value)
    ...
    try {
      return platform === 'ios'
        ...
        : await purchaseStripe(id, stripeHooks)
    }
    catch (e) {
