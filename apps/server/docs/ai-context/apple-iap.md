...
- OpenTelemetry `fluxCredited` counter increment on successful credit (`source: 'apple.iap'`, same pattern as Stripe checkout).
...
After a successful credit, `BillingService.creditFluxFromAppleIapPurchase` increments the shared **`fluxCredited`** OpenTelemetry counter (attribute `source: 'apple.iap'`, `type: 'credit'`). Ledger metadata on `flux_transaction` records `source: 'apple-iap.purchase.completed'` plus `transactionId` / `productId` for audits — same style as Stripe credits, without a separate Redis Stream.
