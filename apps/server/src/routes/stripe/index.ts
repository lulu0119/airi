...
export function createStripeRoutes(
  ...
  return new Hono<HonoEnv>()
    ...
    .post('/checkout', authGuard, rateLimiter({ max: 10, windowSec: 60, metrics: rateLimitMetrics, routeLabel: 'stripe.checkout' }), async (c) => {
      ...
      const redirectBase = resolveTrustedRequestOrigin(c.req.raw, env.DEV_UI_ORIGIN)
      if (!redirectBase) {
    ...
    .post('/portal', authGuard, async (c) => {
      ...
      const portalReturnBase = resolveTrustedRequestOrigin(c.req.raw, env.DEV_UI_ORIGIN)
      if (!portalReturnBase) {
        throw createBadRequestError('Missing trusted request origin', 'INVALID_ORIGIN')
      }
