...
describe('billingService — creditFluxFromAppleIapPurchase', () => {
  ...
  beforeEach(async () => {
    ...
    fluxCreditedAdd.mockClear()
    const metrics = { fluxCredited: { add: fluxCreditedAdd } } as any
    billingService = createBillingService(db, redis, createMockConfigKV(), metrics)
  ...
  it('credits flux, flips fluxCredited, writes a transaction entry, and records OTel credit', async () => {
    ...
    expect(fluxCreditedAdd).toHaveBeenCalledTimes(1)
    expect(fluxCreditedAdd).toHaveBeenCalledWith(1000, { source: 'apple.iap', type: 'credit' })
