...
function buildTrustedWebRedirectUri(redirectUri: string, devUiOrigin: string): string | null {
  try {
    const parsed = new URL(redirectUri)
    if (parsed.pathname !== '/auth/callback')
      return null

    const trustedOrigin = getTrustedOrigin(parsed.origin, devUiOrigin)
    if (!trustedOrigin)
      return null
...
export async function ensureDynamicFirstPartyRedirectUri(
  ...
  switch (clientId) {
    case OIDC_CLIENT_ID_WEB:
      normalizedRedirectUri = buildTrustedWebRedirectUri(redirectUri, devUiOrigin)
      break
