...
export function createSessionState(
  ...
  async function dialUpstream() {
    ...
    try {
      const minimumAmount = await opts.ttsMeter.requiredFlux(userId, STREAMING_PREFLIGHT_CHARS_ESTIMATE)
      await opts.billingService.authorizeFluxSpend({
        userId,
        minimumAmount,
      })
    }
      ...
      // Map 402 to a policy-violation close so the client can show a top-up prompt.
