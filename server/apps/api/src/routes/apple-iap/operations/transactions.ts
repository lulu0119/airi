...
/**
 * Verifies a client-posted StoreKit 2 JWS and applies evidence-first confirmation.
 *
...
export function createTransactionOperation(deps: TransactionOperationDeps) {
  return async (input: { userId: string, body: unknown }) => {
    ...
    const pack = await deps.payment.resolvePack({ provider: 'apple_iap', providerProductId: payload.productId })
    if (!pack)
      throw createBadRequestError('Unknown product', 'UNKNOWN_PRODUCT', { productId: payload.productId })

    const facts = deps.appleAdapter.confirmed({
      transaction: payload,
      userId: input.userId,
      packKey: pack.key,
