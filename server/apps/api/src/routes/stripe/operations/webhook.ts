...
async function handleInvoicePaid(deps: WebhookOperationDeps, invoice: Stripe.Invoice) {
  ...
  if (!planKey && priceId)
    planKey = await deps.payment.resolvePlanKeyFromStripePriceId(priceId) ?? undefined
  ...
  if (!planKey) {
    ...
    const linePriceId = typeof linePrice === 'string'
      ? linePrice
      : linePrice && typeof linePrice === 'object'
        ? linePrice.id
        : undefined
    if (linePriceId)
      planKey = await deps.payment.resolvePlanKeyFromStripePriceId(linePriceId) ?? undefined
  }
  ...
  const plan = await deps.payment.getFluxPlanByKey(planKey)
