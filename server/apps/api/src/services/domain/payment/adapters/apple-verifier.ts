...
/**
 * Creates an Apple IAP JWS and ASSN V2 verifier.
 *
 * Channel routes call this after they receive a client JWS or notification
 * payload. Payment CORE never sees the raw signed string.
 */
export async function createAppleIapVerifier(options: AppleIapVerifierOptions) {
  ...
  async function verifyNotification(signedPayload: string): Promise<ResponseBodyV2DecodedPayload> {
    try {
      ...
      logger.withError(error).warn('ASSN V2 verification failed')
      throw createBadRequestError('Signed notification failed verification', 'NOTIFICATION_VERIFICATION_FAILED')
    }
  }

  async function verifyRenewalInfo(jws: string): Promise<JWSRenewalInfoDecodedPayload> {
    try {
      return await verifier.verifyAndDecodeRenewalInfo(jws)
    }
    catch (error) {
      logger.withError(error).warn('JWS renewal info verification failed')
      throw createBadRequestError('Signed renewal info failed verification', 'RENEWAL_INFO_VERIFICATION_FAILED')
    }
  }

  return { verifyTransaction, verifyNotification, verifyRenewalInfo }
}
