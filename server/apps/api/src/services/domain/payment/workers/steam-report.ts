...
/**
 * Pulls Steam GetReport pages and settles Payment CORE pack evidence.
 *
...
async function processReportOrder(
  ...
  try {
    const receipt = evidenceReceiptFromSteamOrder({ order, userId })
    const confirmation = await deps.payment.settle(receipt)
    return confirmation.applied ? 'pack' : 'skipped'
  }
  catch (error) {
    if (error instanceof ApiError && error.errorCode === 'UNKNOWN_PRODUCT') {
      logger.withFields({ transId: order.transid }).warn('Unknown Steam pack item')
      return 'skipped'
    }
    throw error
  }
